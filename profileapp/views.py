...
def profile(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, f'{username}, Your profile is uploaded.')
            return redirect('/')
    else:
    ...
    if request.method =='POST':
        ...
        if form.is_valid():
            ...
            return redirect('login')
             ...
             context = {'form':form}
             messages.info(request, 'Invalid crediantals')
...
def logout_user(request):
    logout(request)
    messages.info(request, 'You logges out successfully')
    return redirect('login')
